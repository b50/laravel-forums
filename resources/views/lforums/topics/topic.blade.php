...
@section('content')
    ...
    <div class="box-header-blue">
        ...
        <ul style="text-align:right">
            @if (Auth::check())
                @if ($topic->following)
                    <li>
                        <a href="{{ route('forums.topics.unfollow', [
                ...
                @else
                    <li>
                        <a href="{{ route('forums.topics.follow', [
                            'topicType' => Request::get('topicType'),
                            'id' => $topic->id, 'slug' => $topic->slug,
                            '_token' => csrf_token()
                        ]) }}">
                ...
                and Request::get('topicType') == 'forums')
                    <li>
                        <a href="{{ route('forums.topics.move',  [
                            'topicType' => Request::get('topicType'),
                            'id' => $topic->id,
                            'slug' => $topic->slug]) }}">
                            _('Move')
                ...
                @if (Bouncer::hasPermission('forums.lock'))
                    <li>
                        <a href="{{ route('forums.topics.lock', [
                            'topicType' => Request::get('topicType'),
                            'id' => $topic->id, 'slug' => $topic->slug,
                            '_token' => csrf_token()
                        ]) }}">
                ...
                @if (Bouncer::hasPermission('forums.sticky'))
                    <li>
                        <a href="{{ route('forums.topics.sticky', [
                            'topicType' => Request::get('topicType'),
                            'id' => $topic->id,
                            'slug' => $topic->slug,
                            '_token' => csrf_token()
                ...
                @endif
                @if (Bouncer::hasPermission('forums.delete')
                or $topic->user_id == Auth::user()->id)
                    <li>
                        <a href="{{ route('forums.topics.delete', [
                            'topicType' => Request::get('topicType'),
                            'id' => $topic->id,
                            'slug' => $topic->slug
                        ]) }}">
                ...
                @if ($topic->favorite)
                    <li>
                        <a href="{{ route('forums.topics.unfavorite', [
                            'topicType' => Request::get('topicType'),
                            'id' => $topic->id,
                            'slug' => $topic->slug,
                            '_token' => csrf_token()
                ...
                @else
                    <li>
                        <a href="{{ route('forums.topics.favorite', [
                            'topicType' => Request::get('topicType'),
                            'id' => $topic->id,
                            'slug' => $topic->slug,
                            '_token' => csrf_token()
    ...
    <div class="posts">
        <? $i = Request::get('page') * Config::get('posts_per_page') + 1 ?>
        @foreach ($topic->posts as $post)
            @include('lforums.topics.post', $post)
            <? $i++ ?>
        @endforeach
    </div>

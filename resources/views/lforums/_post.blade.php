        ...
        <ul class="post-menu">
            ...
            @if ( ! isset($preview))
                @if (Auth::check())
                    ...
                    <li>
                        <a href="{{ route('forums.topics.reply', [\Route::current()->parameter('topicType'), 'id' => $topic->id, 'slug' => $topic->slug, 'quote' => $post->id]) }}">
                            {{ _('Quote') }}
                        </a>
                    </li>
                @endif
                ...
                @if (Auth::user() and (Auth::user()->id == $post->user_id or Bouncer::hasPermission('forums.delete')) and isset($i) and $i > 1)
                    <li>
                        <a href="{{ route('forums.posts.delete', [\Route::current()->parameter('topicType'), 'id' => $post->id]) }}">
                            {{ _('Delete') }}
                        </a>
                    </li>
                @endif
                @if (Auth::user() and (Auth::user()->id == $post->user_id or Bouncer::hasPermission('forums.edit')))
                    <li>
                        <a href="{{ route('forums.posts.edit', [\Route::current()->parameter('topicType'), 'id' => $post->id]) }}">
                            {{ _('Edit') }}
                        </a>
                    </li>
                @endif
                ...
                @if (Auth::check() and Auth::user()->id != $post->user_id)
                    <li>
                        <a href="{{ route('forums.posts.report', [\Route::current()->parameter('topicType'), 'id' => $post->id]) }}">
                            {{ _('Report') }}
                        </a>
                    </li>
                    <li>
                        <a href="{{ route('forums.posts.vote', ['topicType' => \Route::current()->parameter('topicType'), 'id' => $post->id, 'direction' => 'down', '_token' => csrf_token()]) }}" id="voteDown">
                            <i class="fa fa-angle-down"></i>
                        </a>
                    </li>
                    <li>
                        <a href="{{ route('forums.posts.vote', ['topicType' => \Route::current()->parameter('topicType'), 'id' => $post->id, 'direction' => 'up', '_token' => csrf_token()]) }}" id="voteUp">

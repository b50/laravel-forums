...
class EloquentTopic extends \Eloquent implements PathInterface, HasPresenter {
	 ...
	 * @var array
	 */
	public $fillable = ['title', 'slug', 'path', 'created_at', 'user_id'];
	...
	{
		return $this->hasMany('LaravelForums\Forums\Posts\EloquentPost', 'topic_id')->with('author');
	}
	...
	{
		return $this->belongsTo('LaravelForums\Users\EloquentUser', 'user_id');
	}

	public function favorite()
	{
		return $this->hasOne('LaravelForums\Forums\Topics\Favorite\EloquentFavorite', 'topic_id')
			->where('user_id', \Auth::user()->id);
	}

	public function firstPost()
	{
		return $this->hasOne('LaravelForums\Forums\Posts\EloquentPost', 'topic_id')
	...
	{
		return 'LaravelForums\Forums\Topics\TopicPresenter';
	}

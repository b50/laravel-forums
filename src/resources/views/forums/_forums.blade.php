	...
	@if (preg_match("~$path/.$~", $forum->path))
		<tr>
			...
			<td class="last-forum">
				...
				<div class="last-info">
						{{ HTML::linkRoute('forums.posts.show', $forum->last_topic_title, ['topicType' => 'forums', 'id' => $forum->last_post]) }}<br>
						<span class="description">{{ $forum->last_topic_updated_at }} by </span>
						{{ HTML::linkRoute('users.show', $forum->last_user_username, ['slug' => $forum->last_user_slug]) }}<br>

...
use LaravelForums\Forums\Posts\Create\CreatePostRequest;
use LaravelForums\Forums\Posts\Create\PostCreateListener;
...
class ReplyTopicsController extends BaseController implements PostCreateListener {
	...
	{
		return Redirect::route('forums.topics.reply', ['topicType' => \Route::current()->parameter('topicType'),
			'id' => $this->topic->id, 'slug' => $this->topic->slug])->withInput()->withErrors($errors);
	}
